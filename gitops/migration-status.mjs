...
let sites = YAML.parse(
  await fs.readFileSync("../infrastructure/environments/dplplat01/sites.yaml", "utf-8")
).sites

let siteResults = await Promise.all(
  Object.entries(sites).map(async ([name, site]) => {
...

    {
      ...
      const goDomains = domains.map((d) =>
        d.startsWith("www") ? `www.go${d.slice(3)}` : `go.${d}`
      )
...

    {
      ...
      const result = await Promise.all(
        status.domains.map(async (domain) => {
          try {
            ...
            console.log(
              `${domain} failed DNS lookup - probably not registered or not propergated`,
              error
            )
            return { [domain]: null }
          }
      ...
      Object.entries(domains).forEach(
        ([domain, ip]) =>
          (status.domains[domain] = { ip: ip === dplplat01Ip ? `${dplplat01Ip} !!!` : ip })
      )
...

    {
      ...
      const result = await Promise.all(
        Object.keys(status.domains).map(async (domain) => {
          try {
            const response =
              await $`curl -w "%{http_code}" -o /dev/null -s https://${dplplat01Ip} --insecure -H "Host: ${domain}"`
...

    {
      ...
      const result = await Promise.all(
        Object.keys(status.domains).map(async (domain) => {
          try {
            const response =
              await $`curl -w "%{http_code}" -o /dev/null -s https://${dplplat02Ip} --insecure -H "Host: ${domain}"`
            const statusCode = response.stdout.trim()
            if (["200", "301", "302", "308"].includes(statusCode)) {
...

    {
      ...
      if (status.dns !== "migrated" && status.dplplat01 !== "running") {
        actions.push(
          "Not migrated, proxy is required, but not fully running. Start the proxy on dplplat01 immediately!"
        )
      }
      ...
      if (status.dns === "partially migrated") {
        actions.push(
          "Notify the site owner to migrate fully to dplplat02, or remove the non-migrated domains from the site configuration"
        )
      }
