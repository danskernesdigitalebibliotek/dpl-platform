...
echo(
  chalk.yellow(
    "Will now patch lagoon-env for every environment with the connection details for the internal DB"
  )
)
echo("")

const sites =
  await $`cat ../../../host_mount/environments/dplplat01/sites.yaml | yq '.sites | ... comments="" | keys | .[]'`
...
async function isWebmaster(project) {
  const result =
    await $`cat ../../../host_mount/environments/dplplat01/sites.yaml | yq '.sites.${project}.plan'`
  return result.stdout === "webmaster\n" ? true : false
}

async function getDbConnectionInfo(site, environment = "main") {
  const password =
    await $`kubectl get secret -n ${site}-${environment} database-secret --template={{.data.password}} | base64 -d`
  const service =
    await $`kubectl get mariadbconsumer -n ${site}-${environment} mariadb --template={{.spec.consumer.services.primary}}`
  const readReplicaServices =
    await $`kubectl get mariadbconsumer -n ${site}-${environment} mariadb --template={{.spec.consumer.services.replicas}}`
  const readReplicaService = readReplicaServices
    .text()
    .substring(1, readReplicaServices.text().length - 1)
