...
if (!nginxBackup && !dbBackup) {
  throw Error(
    "No backup files provided. Use --fileBackup and/or --dbBackup to provide paths to either Nginx backup file or database backkup file"
  )
}
...
async function importBackupIntoDatabase(file, connectionInfo, project, environment) {
  ...
  try {
    uncompressedBackup =
      await $`kubectl exec -n ${project}-${environment} deploy/cli -- bash -c "tar -zxvf /tmp/${file} --directory /tmp"`
  } catch (error) {
    echo(error)
  ...
  const { databaseName, databaseHost, databaseUser, databasePassword } = connectionInfo
