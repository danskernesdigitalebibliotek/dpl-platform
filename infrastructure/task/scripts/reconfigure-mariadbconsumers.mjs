...
const blackList = ["customizable-canary", "staging", "canary", "staging", "bibliotek-test", "bnf"]
...
async function reconfigureMariaDbConsumerAndServices(site, environmentType = "main") {
  ...
  try {
    ...
    throw Error(
      `failed to deploy ${site}-${environmentType} post updating MariaDbConsumer and services`
    )
  }
}
