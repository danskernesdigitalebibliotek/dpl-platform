...
await importMainDumpIntoModuletestDatabase(
...
async function runDrushDeployForStateTransferToTakeEffect(targetNamespace) {
  ...
  try {
    ...
    throw Error(`Failed to run drush deploy from CLI pod in namespace ${targetNamespace}`, {
      cause: error,
    })
  }
}
...
async function syncFileFromSourceToTarget(sourceNamespace, targetNamespace) {
  ...
  try {
    await $`kubectl exec -n ${targetNamespace} deployment/cli -- bash -c "rsync --omit-dir-times --recursive --no-perms --no-group --no-owner --no-times --chmod=ugo=rwX --delete --exclude=/styles/* --delete-excluded ${sourceNamespace}@${sshHost}:/app/cms/web/sites/default/files/ /app/cms/web/sites/default/files/"`
  } catch (error) {
    echo(`Failed to sync files from ${sourceNamespace} to ${targetNamespace}`, error.stderr)
    throw Error(`Failed to sync files from ${sourceNamespace} to ${targetNamespace}`, {
      cause: error,
    })
  }
}
...
async function getDatabaseConnectionInfo(namespace) {
  ...
  const databaseConnectionInfo = {
    ...
    databaseHost:
      await $`kubectl get svc ${data.MARIADB_HOST} -n ${namespace} --template={{.spec.externalName}}`,
...
async function makeDatabaseDump(databaseConnectionInfo, sourceNamespace, targetNamespace) {
  ...
  const { databaseName, databaseHost, databaseUser, databasePassword } = databaseConnectionInfo
  ...
  try {
    ...
    echo(
      `Failed to make database dump from ${sourceNamespace} to CLI pod in ${targetNamespace}`,
      error.stderr
    )
    throw Error(
      `Failed to make database dump from ${sourceNamespace} to CLI pod in ${targetNamespace}`,
...
async function importMainDumpIntoModuletestDatabase(
