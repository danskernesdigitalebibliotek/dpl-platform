...
async function runDrushDeployForStateTransferToTakeEffect(targetNamespace) {
  ...
  try {
    ...
    echo(
      `Failed to run drush deploy from CLI pod in target namespace ${targetNamespace}`,
      error.stderr
    )
    throw Error(`Failed to run drush deploy from CLI pod in target namespace ${targetNamespace}`, {
      cause: error,
    })
  }
}
